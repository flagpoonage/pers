...
For more info, visit https://reactjs.org/link/mock-scheduler`);
  ...
  function createMessage(userId, message, isCommand = false) {
    return {
      id: v4_default(),
      time: new Date(),
      userId,
  ...
  // src/ControllerContext.tsx
  var import_react = __toModule(require_react());

  // src/domain/emitter.ts
  function createEmitter() {
    return {
      handlers: new Map()
    };
  }
  function addHandler(emitter, key, handler) {
    const handlers = emitter.handlers.get(key) ?? [];
    handlers.push(handler);
    emitter.handlers.set(key, handlers);
  }
  function removeHandler(emitter, key, handler) {
    const handlers = emitter.handlers.get(key);
  ...
  // src/domain/message-group.ts
  function createMessageGroup(user, messages) {
    if (messages?.find((a) => a.userId !== user)) {
      throw new Error("Trying to create a message group with messages from multiple users");
    }
    return {
      id: v4_default(),
      userId: user,
  ...
  function getGroupsEarliestMessage(messageGroup) {
    return messageGroup.messages[0];
  }
  function getGroupsLatestMessage(messageGroup) {
    const messagesCount = messageGroup.messages.length;
    return messageGroup.messages[messagesCount - 1];
  }
  function getGroupsLatestMessageTime(messageGroup) {
    return getGroupsLatestMessage(messageGroup).time;
  }
  function getGroupsEarliestMessageTime(messageGroup) {
    return getGroupsEarliestMessage(messageGroup).time;
  }
  function insertMessageInGroup(messageGroup, messageToInsert) {
  ...
  var Ok = Symbol("OK");
  var LaterThanAll = Symbol("LaterThanAll");
  var MissingGroup = Symbol("MissingGroup");
  var EarlierThanAll = Symbol("EarlierThanAll");
  ...
  function removeConversationChangeListener(conversation, listener) {
    removeHandler(conversation.emitter, "change", listener);
  }
  function triggerChange(conversation) {
    conversation.messageGroups = [...conversation.messageGroups];
    conversation.emitter.handlers.get("change")?.forEach((a) => a(conversation));
  }
  function getConversationsEarliestMessageGroup(conversation) {
    return conversation.messageGroups[0];
  }
  function getConversationsLatestMessage(conversation) {
    return getGroupsLatestMessage(getConversationsLatestMessageGroup(conversation));
  }
  function getConversationsLatestMessageTime(conversation) {
    return getConversationsLatestMessage(conversation).time;
  }
  function getConversationsLatestMessageGroup(conversation) {
    const groupLength = conversation.messageGroups.length;
    return conversation.messageGroups[groupLength - 1];
  }
  function conversationHasMessages(conversation) {
  ...
  function insertMessageInConversation(conversation, message) {
    if (!conversationHasMessages(conversation)) {
      addLatestMessageGroupToConversation(conversation, createMessageGroup(message.userId, [message]));
    } else {
      const result = getConversationMessageGroupAtTime(conversation, message.time);
      switch (result.state) {
        case Ok:
        ...
        case EarlierThanAll:
          ...
          break;
        case MissingGroup: {
          const missingGroup = findMissingGroupInConversation(conversation, message, result.value);
          if (typeof missingGroup === "number") {
            addMessageGroupToConversationAtIndex(conversation, createMessageGroup(message.userId, [message]), missingGroup);
          } else {
  ...
  function insertLatestMessageInConversation(conversation, message) {
    const latestGroup = getConversationsLatestMessageGroup(conversation);
    if (message.userId !== latestGroup.userId) {
      return addLatestMessageGroupToConversation(conversation, createMessageGroup(message.userId, [message]));
    }
    insertMessageInGroup(latestGroup, message);
    return conversation;
  }
  function insertEarliestMessageInConversation(conversation, message) {
    const earliestGroup = getConversationsEarliestMessageGroup(conversation);
    if (earliestGroup.userId !== message.userId) {
      return addEarliestMessageGroupToConversation(conversation, createMessageGroup(message.userId, [message]));
    }
    insertMessageInGroup(earliestGroup, message);
    return conversation;
  }
  function findMissingGroupInConversation(conversation, message, groupRangeHint) {
  ...
  function addMessageGroupToConversationAtIndex(conversation, messageGroup, index) {
    conversation.messageGroups.splice(index, 0, messageGroup);
    return conversation;
  }
  function addLatestMessageGroupToConversation(conversation, messageGroup) {
    conversation.messageGroups.push(messageGroup);
    return conversation;
  }
  function addEarliestMessageGroupToConversation(conversation, messageGroup) {
  ...
  function getSystemIntroductionText() {
    return `
Welcome to Talk.
...
* 'notif' takes you to the notifications list 
  ...
  function createDefaultCommandEntryOptions() {
    return {
      mask: false
    };
  }

  // src/domain/programs/program-utils.ts
  ...
  // src/domain/programs/set-color.ts
  async function* setColor(controller) {
    let color = yield {
  ...
  // src/domain/programs/set-cmd-color.ts
  async function* setCmdColor(controller) {
    let color = yield {
  ...
  // src/domain/programs/set-sys-color.ts
  async function* setSysColor(controller) {
    let color = yield {
      message: "Please choose a colour for the system user",
      isValidYield: true
    };
...
${output}`,
 ...
 ${exception.message}`,
  ...
  function triggerChange2(controller, key) {
    const handlers = controller.emitter.handlers.get(key);
    if (handlers && handlers.length > 0) {
      handlers.forEach((handler) => handler(controller));
    }
  }
  function triggerSelfChange(controller) {
    triggerChange2(controller, "change_self");
  }
  function triggerUsersChange(controller) {
    triggerChange2(controller, "change_users");
  }
  function triggerCommandColorChange(controller) {
    triggerChange2(controller, "change_command_color");
  }
  function triggerCommandEntryChange(controller) {
    triggerChange2(controller, "change_command_entry");
  }
  function setCommandColor(controller, color) {
    controller.commandColor = color;
    triggerCommandColorChange(controller);
  }
  function setSelfUserProperties(controller, data) {
  ...
  function setSystemUserProperties(controller, data) {
  ...
  function createController() {
    ...
    return {
      ...
      users: new Map([[SystemUser.userId, SystemUser]]),
      commandColor: "#00cfff",
  ...
  function watchControllerEvent(controller, key, handler) {
    addHandler(controller.emitter, key, handler);
  }
  function unwatchControllerEvent(controller, key, handler) {
    removeHandler(controller.emitter, key, handler);
  }
  function getConversationFromController(controller, conversationId) {
    return controller.conversations.get(conversationId);
  }
  function getCurrentConversationFromController(controller) {
    return getConversationFromController(controller, controller.currentConversation);
  }
  async function sendMessageToController(controller, message) {
    ...
    if (isCommandMessage || isCommandMode) {
      ...
      insertMessageInConversation(conversation, createMessage(controller.currentUser.userId, message));
    }
  }
  function parseArguments(command) {
    ...
    do {
      ...
      matches.push(nextValue.startsWith('"') ? nextValue.slice(1, nextValue.length - 1) : nextValue);
  ...
  async function sendCommandToController(controller, conversation, command) {
    ...
    if (!program) {
      insertMessageInConversation(conversation, createMessage(SystemUser.userId, "Invalid command. Type 'commands' for a list of available commands"));
      return;
    }
  ...
  function ControllerProvider({
    ...
    const controller = (0, import_react.useRef)(createController());
    return /* @__PURE__ */ import_react.default.createElement(ControllerContext.Provider, {
      value: controller.current
    }, children);
  }
  function useController() {
    const controller = (0, import_react.useContext)(ControllerContext);
    if (!controller) {
      throw new Error("Using [useCurrentConversation] outside of <ControllerProvider>");
    }
  ...
  function useCurrentConversation() {
    ...
    const [currentConversation, setCurrentConversation] = (0, import_react.useState)(getCurrentConversationFromController(controller));
    ...
    (0, import_react.useEffect)(() => {
      const handler = (controller2) => setCurrentConversation(getCurrentConversationFromController(controller2));
  ...
  function useCurrentConversationMessageGroups() {
    const { conversation } = useCurrentConversation();
    const [messageGroups, setMessageGroups] = (0, import_react.useState)(conversation.messageGroups);
    (0, import_react.useEffect)(() => {
      const changeListener = (c) => setMessageGroups(c.messageGroups);
  ...
  function useAvailableUsers() {
    const { controller } = useController();
    const [users, setUsers] = (0, import_react.useState)(controller.users);
    (0, import_react.useEffect)(() => {
      const handler = (controller2) => setUsers(new Map(controller2.users));
  ...
  function useCommandColor() {
    const { controller } = useController();
    const [commandColor, setCommandColor2] = (0, import_react.useState)(controller.commandColor);
    (0, import_react.useEffect)(() => {
      const handler = (controller2) => setCommandColor2(controller2.commandColor);
  ...
  function useCommandEntryOptions() {
    const { controller } = useController();
    const [commandEntryOptions, setCommandEntryOptions] = (0, import_react.useState)(controller.commandEntryOptions);
    (0, import_react.useEffect)(() => {
      const handler = (controller2) => setCommandEntryOptions({ ...controller2.commandEntryOptions });
  ...
  function useSelf() {
    const { controller } = useController();
    const [self, setSelf] = (0, import_react.useState)(controller.currentUser);
    (0, import_react.useEffect)(() => {
      const handler = (controller2) => setSelf({ ...controller2.currentUser });
  ...
  function Content() {
    ...
    return /* @__PURE__ */ import_react2.default.createElement("div", {
      ...
      return /* @__PURE__ */ import_react2.default.createElement("div", {
        key: group.id,
        className: `pad-y2 clr-white-d5`
      }, /* @__PURE__ */ import_react2.default.createElement("div", {
        className: `no-shrink no-grow w-14 ${group.userId === self.userId ? "clr-trim" : ""} ellipsis`,
        style: group.userId === self.userId ? { color: self.userColor } : { color: users.get(group.userId)?.userColor }
      }, user?.userName), group.messages.map((message) => /* @__PURE__ */ import_react2.default.createElement("div", {
        key: message.id,
        className: "flex row"
      }, /* @__PURE__ */ import_react2.default.createElement("div", {
        className: "no-shrink no-grow w-14 clr-white-d5 ellipsis"
      }, formatMessageTime(message)), /* @__PURE__ */ import_react2.default.createElement("div", {
        className: "message-margin pad-l ws-preserve",
        style: {
          borderLeft: `dashed 1px ${user?.userColor ?? "transparent"}`,
  ...
  function Input() {
    ...
    }, isCommand ? entryOptions.label ?? "Command" : self.userName), /* @__PURE__ */ import_react3.default.createElement("input", {
  ...
  // src/App.tsx
  function App() {
    return /* @__PURE__ */ import_react4.default.createElement(ControllerProvider, null, /* @__PURE__ */ import_react4.default.createElement("div", {
      className: "flex col h"
    }, /* @__PURE__ */ import_react4.default.createElement(Content, null), /* @__PURE__ */ import_react4.default.createElement(Input, null)));
  }
  ...
  ReactDOM.render(/* @__PURE__ */ import_react5.default.createElement(App, null), getRoot());
})();
/*
object-assign
(c) Sindre Sorhus
